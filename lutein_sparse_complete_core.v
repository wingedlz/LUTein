...
module lutein_sparse_complete_core_4x4 #(
    ...
    // -----------------------------------------
    // Sparse indexed weight bank
    // sparse_weight_bank[col][row_idx]
    // -----------------------------------------
    ...
    // -----------------------------------------
    // Front-end zero suppression
    // one row input enters at col0
    // -----------------------------------------
    reg [IN_W-1:0] front_data [0:ROWS-1];
    reg [IDX_W-1:0] front_idx [0:ROWS-1];
    reg             front_valid [0:ROWS-1];
        ...
        if (!rst_n) begin
            ...
            for (r = 0; r < ROWS; r = r + 1) begin
                ...
                for (i = 0; i < IN_LANES; i = i + 1) begin
                    if ($signed(left_data[r*IN_W + i*ACT_W +: ACT_W]) != 0)
                        row_all_zero = 1'b0;
                end

                if (left_valid[r]) begin
                    front_data[r] <= left_data[r*IN_W +: IN_W];
                    front_idx[r]  <= r[IDX_W-1:0];
                    if (sparse_mode_en)
                        front_valid[r] <= ~row_all_zero;
                    else
                        front_valid[r] <= 1'b1;
                end else begin
                    front_valid[r] <= 1'b0;
    ...
    // -----------------------------------------
    // Systolic-like pipeline: data and row_idx
    // move to the right every cycle
    // -----------------------------------------
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // inject into column 0
            for (r = 0; r < ROWS; r = r + 1) begin
                pipe_data[r][0]  <= front_data[r];
                pipe_idx[r][0]   <= front_idx[r];
                pipe_valid[r][0] <= front_valid[r];
            end
...
module tb_lutein_sparse_complete_core_4x4;
    ...
    lutein_sparse_complete_core_4x4 #(
        .ROWS(ROWS),
        .COLS(COLS),
                ...
                for (c = 0; c < COLS; c = c + 1) begin
                    for (j = 0; j < OUT_CH; j = j + 1) begin
                        if (row_zero) begin
                            ...
                            sum = '0;
                            for (i = 0; i < IN_LANES; i = i + 1) begin
                                sum = sum + (in_vec[r][i] * sparse_w[c][r][i][j]);
                            end
                            expected[r][c][j] = sum;
                        end
                    end
    ...
    initial begin
        $dumpfile("sparse_complete_core_wave.vcd");
        $dumpvars(0, tb_lutein_sparse_complete_core_4x4);
        ...
        $display("[DONE] sparse complete core tb finished");
        $finish;
    end
